...
---@return string? input
function InputFrame.get_any_input()
...
---@param input string
---@return string
function InputFrame.get_input_name(input)
    if input == "unbound" then
        return "$menuoptions_configurecontrols_action_unbound"
    end
...
local detect_key = {}
local disable_button = {}
function InputFrame.mod_setting_input(mod_id, gui, in_main_menu, im_id, setting)
    GuiOptionsAddForNextWidget(gui, GUI_OPTION.Layout_NextSameLine)
    GuiOptionsAddForNextWidget(gui, GUI_OPTION.DrawSemiTransparent)
    local uioffset = mod_setting_group_x_offset
    local offset = mod_setting_group_x_offset + GuiGetTextDimensions(gui, setting.ui_name) + 10
    if setting.custom_offset then
        offset = mod_setting_group_x_offset + setting.custom_offset(gui) + 10
        local w = GuiGetTextDimensions(gui, setting.ui_name)
        uioffset = mod_setting_group_x_offset + setting.custom_offset(gui) - w
    end
    GuiText(gui, uioffset, 0, setting.ui_name)
    ...
    if detect_key[setting.id] then
        ...
        text = InputFrame.get_input_name(ModSettingGetNextValue(mod_setting_get_id(mod_id, setting)))
    end
    if disable_button[setting.id] and (InputFrame.read_input_up("Mouse_left") or InputFrame.read_input_down("Mouse_right") or InputFrame.read_input_down("JOY_BUTTON_0") or InputFrame.read_input_down("JOY_BUTTON_1")) then
        disable_button[setting.id] = false
