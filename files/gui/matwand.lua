...
dofile_once("mods/conjurer_reborn/files/wands/matwand/helpers.lua");
dofile_once("mods/conjurer_reborn/files/scripts/lists/brushes.lua");
...
function render_material_picker()
  ...
  Horizontal(1, 1, function()
    for i, category in ipairs(ALL_MATERIALS) do
      ...
      Background({margin=0, style=style}, function()--上面的选择项
  ...
  -- Render material buttons 渲染材料按钮
...
function render_matwand()
  Vertical(main_menu_pos_x, main_menu_pos_y, function()
    render_matwand_buttons()--左边按钮
  end)

  if render_active_matwand_overlay ~= nil then
    Vertical(sub_menu_pos_x, sub_menu_pos_y, function()--右边自动框
