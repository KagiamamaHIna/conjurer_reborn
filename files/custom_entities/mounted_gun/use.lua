...
dofile_once("mods/conjurer_reborn/files/custom_entities/mounted_gun/toggle_use.lua")
...
if is_holding_m1(true) and GameGetFrameNum() % EVERY_N_FRAMES == 0 then
  ...
  shoot_projectile(
    player,
    "mods/conjurer_reborn/files/custom_entities/mounted_gun/bullet.xml",
    shoot_x, shoot_y,
    vel_x, vel_y,
    true
  )

  local muzzle = EntityLoad("mods/conjurer_reborn/files/custom_entities/mounted_gun/muzzle_flash.xml", shoot_x, shoot_y)
